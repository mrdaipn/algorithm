def pylons(distribution_range, cities):
    ...
    while start_index < len(cities):
        candidate = find_possible_farest_installed_plant(
            start_index, distribution_range, cities
        )
        if candidate == -1:
            return -1
...
def find_possible_farest_installed_plant(start_index, distribution_range, cities):
    ...
    while (
        candidate >= max(0, start_index - distribution_range + 1)
        and cities[candidate] == 0
    ):
        candidate -= 1
